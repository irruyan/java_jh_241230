...
<body>
	<h1>JSTL 예제</h1>
	<c:set var="name" value="hong kil-dong" />
	<p>My name is ${name }. </p>
	${str }
	<c:out value="${str }"/>
	<br>
	...
	<ul>
		<c:forEach items="${list }" var="fruit" varStatus="vs">
			<li>
				...
				<c:if test="${vs.last }">Soldout</c:if>
			</li>
		</c:forEach>
	</ul>
	<hr>
	...
	<ul>
		<c:forTokens items="${'사과, 바나나, 포도, 오렌지' }" delims="," 
		var="token" varStatus="vs">
			<li>${vs.count }. ${token }</li>
		</c:forTokens>
	</ul>
	<h1>c:url 예제</h1>
	<c:url var="url" value="/send" >
		<c:param name="name" value="hong kin-Dong"/>
