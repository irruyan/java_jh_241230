...
public class HomeController {
	/*
	 * @RequestMapping
	 *  - 처리할 URL 정보를 지정하는 어노테이션
	 *   - 해당 정보와 일치하는 경우 메소드를 호출하여 실행
	 *  value : 처리할 URL 을 지정
	...
	public String home(Model model, String name, Integer age) {
		...
		model.addAttribute("name", "hong kil-dong");
		
		/* 컨트롤러가 Dispatcher Servlet(디스패처)에게 home 이라는 문자열을 반환
		 *  - 디스패처가 View Resolver(뷰 리졸버)에게 home 이라는 문자열을 전달
		 *  - 뷰 리졸버는 설정된 방법에 따라 home을 가공함
		 *   - 뷰 리졸버 설정은 servlet-context.xml에 있음
		 *   - 기본 뷰 리졸버에 의해 /WEB-INF/view/home.jsp가 완성되어
		 *    최종적으로 해당 jsp의 결과 화면을 클라이언트에게 전송
		 * */
		return "home";
	}
	
	
	/* 메소드의 매개변수에 객체를 넣어주면, 맵핑이 되든 안되든 기본 생성자를 이용하여 객체를 받음
	...
	public String forward(PersonDTO person) {
		System.out.println(person);
		/* forward를 이용하여 url을 처리하면
		 * 해당 url로 request 정보도 같이 전송 매개변수로 받은 데이터들도 함께 전송
		 * */
		return "forward:/send";
	}
}
