...
SELECT MV_TITLE 영화제목, MV_DATE 개봉일 FROM MOVIE WHERE MV_STATE = "상영중";
...
#CGV 강남 1관의 3월 11일 16:20에 상영하는 미키 17의 좌석을 조회하는 쿼리
SELECT SE_NAME FROM SEAT
JOIN SCHEDULE ON SD_SC_NUM = SE_SC_NUM
WHERE SE_POS = "Y" AND SD_NUM = 3;

# CGV 강남 1관의 3월 11일 16:20에 상영하는 미키 17의 예약 가능한 좌석을 조회하는 쿼리
# 예매 리스트와 JOIN해서 예매가 안된 좌석을 조회

SELECT * FROM SEAT
JOIN SCHEDULE ON SD_SC_NUM = SE_SC_NUM
LEFT JOIN TICKET ON TI_SD_NUM = SD_NUM
LEFT JOIN TICKET_LIST ON SE_NUM = TL_SE_NUM
WHERE SE_POS = "Y" AND SD_NUM = 3 AND TL_SE_NUM IS NULL;
