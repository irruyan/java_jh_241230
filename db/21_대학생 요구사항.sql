...
전공 등록
 - 전공은 전공명, 전공 코드, 학과 사무실, 학과장을 입력하여 등록
 - 모든 전공은 학과로 구성된다.
 - 전공 코드는 3자리의 정수이다.
...
교수 등록
 ...
 - 교번은 직접 입력하지 않고,  첫 자리는 P, 취임년도 4자리, 전공코드 3자리, 취임순(같은년도) 3자리로 구성

지도 교수 등록
 - 학생은 전공 교수 중 한명이 지도 교수로 배정된다.

과목 등록
  - 과목코드, 과목명, 이수학점, 시간, 타입(전공필수, 전공선택, 교양필수, 교양선택, 교직필수)
  - 과목코드는 분류코드 3자리, 순서 3자리로 구성
  - 분류코드는 교직(TEA), 교양(CUL), 전공별로 할당된 전공 코드를 사용한다.
  - 이수학점은 이수 시 인정되는 학점으로 0~4학점까지
  - 시간은 1주일 강의 시간으로 0~4시간이다.
  - 같은 내용의 수업이라도 전공마다 각각 등록한다.

강의 등록
 - 등록된 과목, 교수를 선택하고 연도, 학기, 시간표, 강의장, 강의계획서, 분반을 입력하여 등록한다.
 - 연도는 강의가 개설되는 연도 4자리이다.
 ...
 수강신청/취소
  - 정원이 다 차지 않은 수업에 한해 수강신청이 가능하다.
  - 한 학생은 같은 기간에 같은 강의를 여러번 수강 신청 할 수 없다.
 
성적 등록
 - 학생이 듣는 강의가 종강되면 교수는 성적을 입력한다.
 - 성적은 A+, A, B+ ...., D, F, PASS, FAIL 로 구성된다.
 - 성적은 중간, 기말, 과제, 출결로 이루어지며 각 비중은 교수가 변경한다.
 - 출결이 4회 이상 결석이면 성적에 상관없이 F이다.
