...
public class Ex01_Client {
	...
	private static Product getProduct(List<Product> list2, String code) {
	...
	private static boolean printProduct(List<Product> list2, String code) {
	...
	private static void login() {
		...
		//객체 정보와 일치하는지 확인해서 있으면 정보를 가져옴
		Member user = getMember(members, member);
		
		if(user==null) {
			System.out.println("아이디 또는 비번이 일치하지 않음");
			return;
		}
	...
	private static Member getMember(List<Member> members2, Member member) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	private static void signup() {
		...
		String id = scan.next();
		System.out.print("비번 : ");
		String pw = scan.next(); // 공백을 허용하지 않을때 next를 씀
		System.out.print("비번 확인 : ");
		String pw2 = scan.next();
		
		//비번과 비번 확인이 일치 하지 않으면 알림 후 종료
		if(!pw.equals(pw2)) {
