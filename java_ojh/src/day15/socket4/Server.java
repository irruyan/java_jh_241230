...
public class Server {
				...
				while(true) {
					...
					synchronized (list) {
						...
						for(ObjectOutputStream client : list) {
							//메세지를 쓴 클라이언트에겐 메시지를 보내지 않음
							if(client != oos && !chat.equals(EXIT)) {
								send(client, id, chat);
							}
							//연결된 클라이언트가 EXIT를 입력했을때
							else if(client == oos && chat.equals(EXIT)) {
								//클라이언트의 receive()에 있는 쓰레드 종료하기 위해서
								send(client, id, chat);
								list.remove(oos);
							}
						}
	...
	private void send(ObjectOutputStream client, String id, String chat) {
		...
		try {
			// 하나의 클라이언트에 여러 메세지를 동시에 보내려고 하면 문제가 발생할 수 있음
			// 아래 동기화를 통해 먼저 들어온 작업부터 실행되도록 해줌
			synchronized (client) {
				client.writeUTF(id);
